jQuery.fn.slideViewerPro = function(settings) {
	  settings = jQuery.extend({
			...
			leftButtonInner: "<img src='/images/left-ui0.png'/ >", //could be an image "<img src='images/larw.gif' />" or an escaped char as "&larr";
			rightButtonInner: "<img src='/images/right-ui0.png'/ >", //could be an image or an escaped char as "&rarr";
			autoslide: false,
			typo: false,
			typoFullOpacity: 0.9,
			shuffle: false
...
///*----------------------------------------------------
// * @author   Bal Andrei <BallBag@mail.ru>
// * @description  внесены изменения под адаптацию Img и самого слайдера под ширину экрана
// * 
// *   --Начало--13.11.2015 16:49
// ----------------------------------------------------*/
//		var pictWidth = container.find("img").width(); 
//		var pictHeight = container.find("img").height();
		
		var nOldPictWidth = container.find("img").width();
		var nOldPictHeight = container.find("img").height();
		
		
		//alert(nOldPictWidth);
		//alert(nOldPictHeight);
		
		var nSliderHeight = $('#Slider').height();
		if($(window).width()<960)
		{
			var nSliderWidth = 960;
		}
		else {
			var nSliderWidth = $(window).width();
		}
		
		
		
		var nRatioHeight = nSliderHeight / nOldPictHeight;
		var nRatioWIdth = nSliderWidth  / nOldPictWidth;
		...
		//var pictHeight = $('#Slider').height();


        /*----------------------------------------------------
         * @author   Bal Andrei <BallBag@mail.ru>
         * @description  изменили высоту, как 90% от высоты экрана
         *
         *   --Начало--22.02.2016 19:41
        ----------------------------------------------------*/
		//var pictHeight = nRatio * nOldPictHeight;

        var pictHeight = nRatio * nOldPictHeight * 0.9;
        
        
        /*------------------------------------------------------
         *  --Конец--22.02.2016 19:41
          -------------------------------------------------------*/
		var pictWidth = nRatio * nOldPictWidth;
		
		//устанавливаем новые значение для ширины и высоты картинки
		//на основе вычиленного ранее соотношения
		container.find("img").width(pictWidth);
        container.find("img").height(pictHeight);
...
//        		 * @description  внесены изменения под адаптацию Img и самого слайдера под ширину экрана
//        		 * 
//        		 *  --Начало--12.11.2015 15:30
//        		 ----------------------------------------------------*/
////        				var pictWidth = container.find("img").width(); 
////        				var pictHeight = container.find("img").height();
        				
//        				//устаналиваем высоту изображения равной высоте слайдера
//        				var pictHeight = $('#Slider').height();
//        		        container.find("img").height(pictHeight);
        		        
//        		        //находим соотношение реальной высоты картинки к высоте слайдера
//        		        var nRatio = container.find("img").height() / pictHeight ;
        		        
//        		        var pictWidth = $(window).width();
//        		        //устанавливаем ширину изображения на основе соотношения реальной высоты картинки и высоты слайдера
//        		        container.find("img").width(container.find("img").width() * nRatio);
        		        
        		        
//        		        //устаналвиваем слайдеру свойство overflow: hidden 
//        		        $('#Slider').css({'overflow' : 'hidden'});
...
//        		 -------------------------------------------------------*/
		...
		container.each(function(i) {
			...
			jQuery("div#thumbSlider" + j).css({left: contBorder, top: settings.thumbsTopMargin+"px", width: settings.thumbs*thumbsWidth+((settings.thumbsRightMargin*settings.thumbs)-settings.thumbsRightMargin), height: thumbsHeight, textAlign: "center", overflow: "hidden", margin: "0 auto"});
